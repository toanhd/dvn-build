...
# channel previously setup in the dvn tutorial and install the
...
set -x
peer channel fetch 0 $CHANNEL_NAME.block -o orderer.toanhd.com:7050 -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
set +x
cat log.txt
