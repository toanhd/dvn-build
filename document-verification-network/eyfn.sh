...
function printHelp () {
  ...
  echo
  echo "	eyfn.sh generate -c dvnchannel"
  echo "	eyfn.sh up -c dvnchannel -s couchdb"
  echo "	eyfn.sh up -l node"
  echo "	eyfn.sh down -c dvnchannel"
  echo
...
# be parked in the dvn folder titled ``crypto-config``.
...
# If dvn wasn't run abort
if [ ! -d crypto-config ]; then
  echo
  echo "ERROR: Please, run dvn.sh first."
  echo
  exit 1
fi
